...
def preparation(details, type_chart, moves):
    ...
    if submit_names:
        def simulation_basedon_moves(details, type_chart, moves, for_, against):
            ...
            st.dataframe(for_moves.loc[:, ["moves", "type", "power", "accuracy"]])
            
            def get_player_moves(for_moves):
                if not for_moves.empty:
                    ...
                    return for_move_data, submitted
                else:
            ...
            for_move_data, submitted = get_player_moves(for_moves)
            if submitted:
        ...
        try:
            for_move_data, for_speed, for_hp, against_move_data, against_speed, against_hp = simulation_basedon_moves(details, type_chart, moves, for_, against)
            ...
            st.warning("Please Click Submit Button")
